...
//   ilog("Processing payroll! Available budget is ${b}", ("b", budget));
   ...
   for( uint32_t i = 0; i < active_workers.size() && budget > 0; ++i )
   {
      ...
      //       Since it's not the case on columbu mainnet, we're not using a check here.
 ...
 * https://github.com/columbu/columbu/issues/868
 *
 ...
 * https://github.com/columbu/columbu/issues/890
 *
...
void process_hf_868_890( database& db, bool skip_check_call_orders )
{
   ...
   for( auto asset_itr = asset_idx.lower_bound(true); asset_itr != asset_idx.end(); ++asset_itr )
   {
      ...
      // https://github.com/columbu/columbu/pull/832#issuecomment-384112633
      ...
      // always update the median feed due to https://github.com/columbu/columbu/issues/890
      ...
      // Note: due to columbu issue #935, the check below (using median_changed) is incorrect.
 ...
 * https://github.com/columbu/columbu/issues/935
 *
 * @param db the database
 */
// TODO: for better performance, this function can be removed if it actually updated nothing at hf time.
